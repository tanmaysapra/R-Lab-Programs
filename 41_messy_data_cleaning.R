...
cat("\n=== (Optional) separate(): Parse 'ExtraInfo' --> Initial, InfoYear,
Section ===\n")
...
# -----------------------------
cat("\n=== 4) spread(): Create year-wise columns for each
Student/Subject ===\n")
...
# -----------------------------
cat("\n=== 6) fill(): Forward-fill missing 'Attendance' within each
StudentID ===\n")
progress <- data.frame(
 StudentID = c(1, 1, 1, 2, 2, 2, 3, 3),
 Term = c("T1", "T2", "T3", "T1", "T2", "T3", "T1", "T2"),
 Attendance = c("Present", NA, NA, "Absent", NA, "Present", NA,
"Present"),
 stringsAsFactors = FALSE
)
...
cat(" long2 rows:", nrow(long2), " | long_clean rows:",
nrow(long_clean), "\n")
cat("\nDone.\n")